...
def test_parse_log_line():
    ...
    expected = LogEntry(
        datetime.fromisoformat('2026-06-13T11:02:21.222819+00:00'), 'INFO', 'order-service', 'Order placed successfully')
...
def test_parse_log_line_with_wrong_no_of_log_items():
    ...
    assert str(exc_info.value) == 'Incorret log format: atleast 4 log-items were expected'

def test_parse_log_line_with_wrong_timestamp():
    log_line = 'wrong_timestamp_format [INFO] service=order-service Order placed successfully'
    with pytest.raises(ValueError) as exc_info:
        parse_log_line(log_line)
...
def test_summarize_logs():
    log_entries = [
        ...
        LogEntry(
            ...
            'user-service', 
            'User logged in')]
    expected = LogSummary({'INFO': 2}, ['Order placed successfully', 'User logged in'], 11)
